...
app.controller("banhang-ctrl", function ($scope, $http) {
  ...
  $scope.createHoaDon = function () {
    ...
    if (isNaN(khachThanhToan) || khachThanhToan === "") {
      $scope.showNotification(
        "Giá trị thanh toán không hợp lệ. Vui lòng nhập số!",
        "error"
      );
      return;
    }
  ...
  $scope.getKhuyenmaiById = function (maKhuyenMai) {
    // Giả sử $scope.tongTienThanhToan() trả về tổng tiền đơn hàng
    var tongTien = $scope.getSum(); // Lấy tổng tiền đơn hàng từ hàm này
    tongTien = parseFloat(tongTien);

    $http
      .get("/don-hang/kiem-tra", {
        params: { maKhuyenMai: maKhuyenMai, tongTien: tongTien }, // Gửi mã khuyến mãi và tổng tiền
      })
      .then(function (response) {
        console.log("Khuyến mãi by id:", response.data);

        // Kiểm tra status từ backend
        if (response.data.status === "error") {
          // Nếu mã khuyến mãi không hợp lệ hoặc có lỗi
          alert(response.data.message); // Hiển thị thông báo lỗi
        } else if (response.data.status === "success") {
          // Nếu mã khuyến mãi hợp lệ
          $scope.khuyenMaiById = response.data.data; // Lưu thông tin khuyến mãi vào biến

          // Cập nhật các giá trị vào form
          $("#ma-khuyen-mai").val(response.data.data.maKhuyenMai);
          if (response.data.data.mucGiamGia < 100) {
            $("#muc-giam-gia").val(response.data.data.mucGiamGia + "%");
          } else {
            $("#muc-giam-gia").val(response.data.data.mucGiamGia);
          }

          // Hiển thị thông báo thành công
          alert("Khuyến mãi áp dụng thành công!");
        }
      })
      .catch(function (error) {
        // Xử lý lỗi nếu có sự cố với yêu cầu HTTP
  ...
  $scope.printer = function () {
    ...
    $http
      .get("/hoa-don/get-invoice/" + idHoaDoncheck)
      .then(function (response) {
  ...
  $scope.validateQuantity = function (details) {
    ...
    let availableProduct = $scope.products.find(
      (product) => product?.idSanPhamChiTiet === details.idSanPham
    );
