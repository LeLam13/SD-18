...
// Hàm chuyển tới trang cuối
    ...
    $scope.updateTT = function (idThuongHieu) {
        if (confirm("Xác nhận đổi?")) {
            var url = "/admin/thuong-hieu/updateTT" + "/" + idThuongHieu;
    ...
    $scope.search = function () {
        const url = `/admin/thuong-hieu/search?page=${$scope.page}&size=${$scope.size}&query=${encodeURIComponent($scope.searchQuery)}`;
        $http.get(url).then(resp => {
            $scope.itemss = resp.data.content;
