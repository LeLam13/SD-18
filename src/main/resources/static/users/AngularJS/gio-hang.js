...
app.controller("gio-hang-ctrl", function ($scope, $http) {
  ...
  $scope.filter = function (filterData) {
    ...
    $http
      .post(
        `/san-pham/chi-tiet/filter?page=${$scope.page}&size=${$scope.size}`,
        filterData
      )
  ...
  $scope.getAllProduct = function () {
    $http
      .get(
        `/san-pham/find-all-san-pham?page=${$scope.page}&size=${$scope.size}`
      )
      .then(function (response) {
  ...
  $scope.createCartWithUsername = function (id) {
    ...
    if (!$scope.items || $scope.items.length === 0) {
      ...
      var itemCartDetail = $scope.items.find(
        (item) => item.sanPhamChiTiet.idSanPhamChiTiet === id
      );
      if (itemCartDetail) {
  ...
  $scope.getDetailCart = function (idGioHang) {
    $http
      .get(`/gio-hang-chi-tiet/${idGioHang}`)
      .then((response) => {
  ...
  $scope.removeLocalStorage = function (id) {
    ...
    if (!$scope.username) {
      ...
      $http({
        method: "DELETE",
        url:
          "/gio-hang-chi-tiet/xoa-theo-id-san-pham/" +
          item.sanPhamChiTiet.idSanPhamChiTiet,
      }).then(
        function (response) {
  ...
  $scope.saveToLocalStorage = function () {
    let cleanedItems = $scope.items.map((item) =>
      JSON.parse(JSON.stringify(item))
    ); // Loại bỏ getter/setter từ từng phần tử
  ...
  function ($scope, $http, $sce, $timeout, $rootScope) {
    ...
    $scope.promotionAmount = function () {
      ...
      if ($scope.discountRate.includes("%")) {
        let reduceMoney =
          parseFloat($scope.discountRate.replace("%", "")) / 100;
    ...
    $scope.createOrderOnline = function () {
      ...
      var selectedProvinceName = $scope.selectedProvince
        ? $scope.selectedProvince.ProvinceName
        : "";

      // Tìm quận/huyện đã chọn
      var selectedDistrict = $scope.districts.find(
        (d) => d.DistrictID == $scope.selectedDistricts
      ); // Sử dụng == thay vì ===
      var selectedDistrictName = selectedDistrict
        ? selectedDistrict.DistrictName
        : "";

      // Tìm xã/phường đã chọn
      var selectedWard = $scope.wards.find(
        (w) => w.WardCode === $scope.selectedWards
      );
      ...
      $scope.orderData = {
        ...
        idKhuyenMai:
          $scope.khuyenMaiById.idKhuyenMai != null
            ? $scope.khuyenMaiById.idKhuyenMai
            : null,
        // orderDetail: $scope.itemsOrder
        orderDetail: $scope.username
          ? $scope.itemsOrder.map((item) => ({
    ...
    $scope.showVNPay = function (response) {
      ...
      $http
        .post("/create-payment", $scope.dataInvoice)
        .then(function (response) {
    ...
    $scope.deleteCartDetail = function (response) {
      ...
      $http
        .delete("/gio-hang/xoa-gio-hang-chi-tiet/" + idGioHang)
        .then(function (response) {
    ...
    $scope.soLuongPlus = function (orderProduct) {
      ...
      var product = $scope.itemsOrder.find(
        (item) => item.idSanPhamChiTiet === orderProduct.idSanPhamChiTiet
      );
    ...
    $scope.validateQuantity = function (orderProduct) {
      ...
      let availableProduct = $scope.listProducts.find(
        (product) =>
          product.idSanPhamChiTiet ===
          orderProduct.sanPhamChiTiet.idSanPhamChiTiet
      );
    ...
    $scope.createInvoince = function (response) {
      ...
      $http
        .post("/don-hang-online/tao-moi-hoa-don", $scope.dataInvoice)
        .then(function (response) {
