...
// Hàm lọc
  ...
  $scope.getUserName = function (){
    $http.get("/lay-tai-khoan").then(function (response){
      console.log("check user1: ",response);
    ...
    if (!$scope.username){
      ...
      if(item){
        ...
        $http.get(`/danh-sach-san-pham/${id}`).then(response =>{
          ...
          $scope.items.push(cleanData); // Lưu dữ liệu sạch vào mảng
          console.log("Updated $scope.items:", $scope.items);
          $scope.saveToLocalStorage(); // Gọi hàm lưu trữ
        });
      }
    }else {
  ...
  $scope.removeLocalStorage = function (id){
    console.log('check delete removeLocalStorage: ',id)
    console.log('check delete removeLocalStorage: ',$scope.items)
    if (!$scope.username){
      var index = this.items.findIndex(item => item.idSanPhamChiTiet == id.idSanPhamChiTiet);
      $scope.items.splice(index,1);
      this.saveToLocalStorage();
    }else {
        var item = this.items.find(item=>item.sanPhamChiTiet.idSanPhamChiTiet === id.sanPhamChiTiet.idSanPhamChiTiet
        );
        ...
        $http({
          ...
          params: {
            idGioHangChiTiet: id.idGioHangChiTiet
          }
        }).then(function(response) {
  ...
  $scope.saveToLocalStorage = function (){
    let cleanedItems = $scope.items.map(item => JSON.parse(JSON.stringify(item))); // Loại bỏ getter/setter từ từng phần tử
    console.log("Cleaned items:", cleanedItems);
    let json = JSON.stringify(cleanedItems);
    console.log("Final JSON data:", json);
    // var json = JSON.stringify(angular.copy(this.items));
    // console.log("check this.items",json);
...
app.controller("don-hang-online-ctrl", function ($scope, $http,$sce,$timeout,$rootScope,$location) {
  ...
  $scope.getCartGH = function (){
    $http.get("/gio-hang/lay-theo-user").then(function (response){
      console.log("check gio hang check check: ",response.data);
      $scope.cart = response.data;
      $scope.getDetailCart($scope.cart.idGioHang);
      idGioHang = $scope.cart.idGioHang;
    }).catch(function (errors) {
  ...
  //lấy giỏ hàng chi tiết
  $scope.getIdGioHangCT = null;
  $scope.getDetailCart = function(idGioHang){
    $http.get(`/gio-hang-chi-tiet/${idGioHang}`).then(response =>{
      response.data.qty = response.data.soLuong;
      // response.data.soLuong = 1;
      console.log("check cart detail get getCartGH123: ",response.data)
      // $scope.getIdGioHangCT = response.data[0].idGioHangChiTiet;
      // console.log("$scope.getIdGioHangCT ",$scope.getIdGioHangCT);
  ...
  $scope.createOrderOnline = function (){
    if ($scope.itemsOrder === null || $scope.itemsOrder.length === 0) {
      $scope.showNotification("Chưa thêm sản phẩm vào giỏ hàng" , 'error');
      return;
    }
    for (let i = 0; i < $scope.itemsOrder.length; i++) {
      let orderProduct = $scope.itemsOrder[i];
      let availableProduct = null;
      if (!$scope.username) {
        availableProduct = $scope.listProducts.find(product => product.idSanPhamChiTiet === orderProduct.idSanPhamChiTiet);
        console.log("not user",availableProduct);
      } else {
        availableProduct = $scope.listProducts.find(product => product.idSanPhamChiTiet === orderProduct.sanPhamChiTiet.idSanPhamChiTiet);
        console.log(" user",availableProduct);
      }

      // Kiểm tra nếu số lượng sản phẩm trong giỏ hàng vượt quá số lượng có sẵn
      if (orderProduct.soLuong > availableProduct.soLuong) {
        $scope.showNotification('Số lượng sản phẩm ' + availableProduct.idSanPham.ten  + ' vượt quá số lượng có sẵn!', 'error');
        return;
      }
    }
  ...
  $scope.soLuongPlus = function (orderProduct) {
    ...
    if (product && availableProduct) {
      ...
      if (product.soLuong >= availableProduct.soLuong) {
        orderProduct.invalidIncrease = true; // Bật trạng thái lỗi
        $scope.showNotification('Số lượng đã đạt giới hạn, không thể tăng thêm!', 'error');
        return;
      }

      // Nếu số lượng có thể tăng, thực hiện tăng
  ...
  $scope.soLuongReduce = function(orderProduct){
    ...
    if(product.soLuong >1){
      ...
      $scope.showNotification('Số lượng không thể giảm xuống mức tối thiểu 1!','error');
    }
  }
  ...
  $scope.removeLocalStorage1 = function (id){
    console.log('check delete removeLocalStorage1: ',id)
    console.log('check delete removeLocalStorage1 2: ',$scope.items)
    if (!$scope.username){
      var index = this.items.findIndex(item => item.idSanPhamChiTiet == id.idSanPhamChiTiet);
      ...
      $http({
        ...
        console.log("Đã xóa sản phẩm khỏi giỏ hàng:", response.data);
        $scope.getDetailCart(response.data.gioHang.idGioHang); // Lấy lại giỏ hàng sau khi xóa
        $rootScope.$broadcast('cartUpdated'); // Cập nhật trạng thái giỏ hàng
      }, function(error) {
        console.error("Lỗi khi xóa sản phẩm:", error.data);
        alert("Không tìm thấy sản phẩm hoặc có lỗi khi xóa!");
      });
  ...
  $scope.validateQuantity = function(orderProduct) {
    // Kiểm tra nếu số lượng không phải là số nguyên dương
    if (!/^\d+$/.test(orderProduct.soLuong) || parseInt(orderProduct.soLuong) <= 0) {
      orderProduct.invalidQuantity = true;
      orderProduct.errorMessage = "Vui lòng nhập số lượng là số nguyên và lớn hơn 0!";
      return;
    }
    // Chuyển số lượng từ chuỗi sang số
    let inputQuantity = parseInt(orderProduct.soLuong);
    ...
    if (availableProduct && inputQuantity > availableProduct.soLuong) {
      orderProduct.invalidQuantity = true;
      orderProduct.errorMessage = "Số lượng nhập vượt quá số lượng có sẵn!";
    } else {
      orderProduct.invalidQuantity = false;
      orderProduct.errorMessage = "";
      console.log("check2:");
